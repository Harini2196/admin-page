...
export class CreateBatchComponent implements OnInit {
  ...
  addBatch: FormGroup = this.fb.group({
    ...
    status: ['Yet to start', Validators.required]
  });
  isEdit: boolean;
  batchId: string;
  ...
  async getUsersBasedOnBatch(courseId = null) {
    try {
      ...
      if (users["status"] == 200) {
        ...
        this.autoCompleteStudents = [...this.studentsData];
      }
    } catch (error) {
      console.log(error);
    } finally {
 ...
 async suspendUser(checked,user) {
  ...
  addStudent() {
    ...
    if (this.studentDetail) {
      this.students.unshift({ ...this.studentDetail, isNew: true });
      this.autoCompleteStudents.forEach((student, index) => {
        if (student.email === this.studentDetail.email) {
          this.autoCompleteStudents.splice([index], 1);
        }
      });
      this.dataSource.data = this.students;
      this.dataSource.paginator = this.paginator;
  ...
  async getBatchDetails(id) {
    try {
      this.students = [];
      const batch = await this.adminService.getBatchDetailById(id).toPromise();
      if (batch["status"] == 200) {
        console.log(batch["body"]);
        this.addBatch.patchValue({ ...batch["body"] });
        this.students = batch["body"]["students"].map((s) => {
          let student = this.allStudents.find(
        ...
        this.getUsersBasedOnBatch(batch['body']['courseId']);
        this.courseId = batch['body']['courseId'];
        this.dataSource.data = this.students;
        this.dataSource.paginator = this.paginator;
      }
    } catch (error) {
