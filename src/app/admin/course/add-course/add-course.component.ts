...
export class AddCourseComponent implements OnInit {
  ...
  enablePositiveNums(e) {
    if (!((e.keyCode > 95 && e.keyCode < 106) || (e.keyCode > 47 && e.keyCode < 58) || e.keyCode === 8)) {
      return false;
    }
  }
  getFeatureErrorStatus(index) {
    return this.addCourse.controls.basic['controls'].features.controls[index];
  }
  getTopicErrorStatus(index) {
    return this.addCourse.controls.curriculum['controls'][index].controls.title;
  }
  getSubTopicErrorStatus(index, sIndex) {
    return this.addCourse.controls.curriculum['controls'][index].controls.topics.controls[sIndex].controls.title;
  }
  getFaqErrorStatus(index, field) {
    return this.addCourse.controls.faqs['controls'][index].controls[field];
  }
  trim(fieldName) {
    return this.addCourse.controls.basic['controls'][fieldName].setValue(this.addCourse.value.basic[fieldName].trim());
  }
}
